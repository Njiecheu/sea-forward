...
- **Extractors** (in `postprocess`) build a labelled `xarray.DataArray` — they decide *what* data (which variable, depth, time).
- **Plotters** (in `plotting`) decide *how* it looks (colormap, range, title) — reading the labels from the data by default.
