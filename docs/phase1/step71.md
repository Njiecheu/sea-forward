...
!!! important
    `install/00_download_libraries.sh` does exactly these `wget`s if you prefer to run the script. **MPI/OpenMPI is not needed** for the sequential stack; it's only for a parallel build (see `install/notes/README_parallel.md`).
